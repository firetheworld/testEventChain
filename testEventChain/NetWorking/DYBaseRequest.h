...
typedef NS_ENUM(NSInteger, DYResponseSerializerType) {
	...
	/// Protobuf type
	DYResponseSerializerTypeProtobuf,
};

///  Request priority
typedef NS_ENUM(NSInteger, DYRequestPriority) {
	DYRequestPriorityLow = -4L,
	DYRequestPriorityDefault = 0,
	DYRequestPriorityHigh = 4,
};

///  The DYRequestDelegate protocol defines several optional methods you can use
///  to receive network-related messages. All the delegate methods will be called
///  on the main queue.

@class DYBaseRequest;

@protocol DYRequestDelegate <NSObject>

@optional
///  Tell the delegate that the request has finished successfully.
///
///  @param request The corresponding request.
- (void)requestFinished:(__kindof DYBaseRequest *)request;

///  Tell the delegate that the request has failed.
///
///  @param request The corresponding request.
- (void)requestFailed:(__kindof DYBaseRequest *)request;

@end

///  The DYRequestAccessory protocol defines several optional methods that can be
...
@protocol DYRequestAccessory <NSObject>
...
@interface DYBaseRequest : NSObject
...
///  `DYResponseSerializerType`. Note this value can be nil if request failed.
///
...
///  If you use `DYResponseSerializerTypeJSON`, this is a convenience (and sematic) getter
...
///  Default is nil.
@property (nonatomic, weak, nullable) id<DYRequestDelegate> delegate;

///  This can be used to add several accossories object. Note if you use `addAccessory` to add acceesory
///  this array will be automatically created. Default is nil.
@property (nonatomic, strong, nullable) NSMutableArray<id<DYRequestAccessory>> *requestAccessories;
...
///  The priority of the request. Effective only on iOS 8+. Default is `DYRequestPriorityDefault`.
@property (nonatomic) DYRequestPriority requestPriority;
...
/// This method will return mock data when test
- (id)mockData;
